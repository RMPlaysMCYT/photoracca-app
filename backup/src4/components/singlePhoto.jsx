    ...
    takePhoto: () => {
      ...
      const timer = setInterval(() => {
        ...
        if (count <= 0) {
          ...
          // Draw the video to canvas. If mirrorPreview is enabled, flip horizontally
          ctx.save();
          if (mirrorPreview) {
          ...
          try {
            ...
            // If preview should be mirrored, flip drawing horizontally so previewData matches on-screen preview
            pCtx.save();
            if (mirrorPreview) {
              // flip full preview canvas horizontally; adjust destination x accordingly
              pCtx.translate(PREVIEW_W, 0);
              pCtx.scale(-1, 1);
              // when flipped, draw at mirrored x position
              pCtx.drawImage(video, 0, 0, vw, vh, PREVIEW_W - dx - drawW, dy, drawW, drawH);
            } else {
...
// set displayName (eslint react/display-name) and propTypes (eslint react/prop-types)
