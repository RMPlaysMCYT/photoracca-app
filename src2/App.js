...
function App() {
  ...
  const TakeMultiplePhotosWidth = async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Add separator lines between photos (except after the last one)
      if (i < 2) {
  ...
  const TakeMultiplePhotos2 = async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Add separator lines between frames (except after the last one)
      if (i < 2) {
  ...
  const getCanvasClassName = () => {
    const baseClass = "canvas-base";
    
    if (!hasPhotoShotted) return baseClass;
    
    switch (photoMode) {
      case "multiple":
        return `${baseClass} canvas-multiple`;
      case "multiple2":
        return `${baseClass} canvas-multiple2`;
      default:
        return `${baseClass} canvas-classic`;
    }
  };
        ...
        <div>
            ...
            <div className="Buttons">
              <select
                ...
                onChange={(e) => {
                  ...
                  // Clear any existing photo when mode changes
                  if (hasPhotoShotted) {
                    ClosePhoto();
                  }
                }}
          ...
          <div className={`Resultas ${hasPhotoShotted ? "hasPhotoShotted" : ""}`}>
            <div className="canvas-container">
              <canvas className={getCanvasClassName()} ref={photoReferencial}></canvas>
            </div>
            {hasPhotoShotted && (
