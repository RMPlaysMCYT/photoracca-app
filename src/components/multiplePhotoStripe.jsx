 ...
 */
const MultiplePhotoStripe = forwardRef(({ videoRef, canvasRef, countdown = 3, maxPhotos = 4 }, ref) => {
  ...
  const composeStrip = async (framedArray, spacing = 12, background = "#fff") => {
    ...
    const totalHeight = imgs.reduce((s, i) => s + i.height, 0) + spacing * (imgs.length - 1);
    const canvas = document.createElement("canvas");
    canvas.width = width;
  ...
  return (
    <div className="multiplePhotoStripe" style={{ padding: 12 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <button
          onClick={async () => {
            const s = await (ref && ref.current && ref.current.saveStrip ? ref.current.saveStrip() : null);
            if (!s) {
              // fallback: compose locally
              const strip = await composeStrip(framedPhotos.length === photos.length ? framedPhotos : photos);
              if (strip) downloadDataUrl(strip, "photoracca_strip.png");
            }
          }}
        >
      ...
      <div style={{ marginTop: 12 }}>
        {running && currentCount > 0 && (
          <div style={{ fontSize: 48, fontWeight: "bold", color: "#e53935" }}>{currentCount}</div>
        )}

        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          {(framedPhotos.length > 0 ? framedPhotos : photos).map((p, idx) => (
            <div key={idx} style={{ width: 120, position: "relative" }}>
              <img src={p} alt={`shot-${idx}`} style={{ width: "100%", display: "block" }} />
            </div>
          ))}
        </div>
      </div>
