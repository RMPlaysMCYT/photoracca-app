...
const MultiplePhotoStandard = forwardRef(
  ...
  const borderPx = Math.round((borderMm / 25.4) * ppi);
      ...
      for (let i = 0; i < slots; i++) {
        ...
        const { sw, sh, sx, sy } = coverSourceRect(img.width, img.height, innerW, innerH);
    ...
    return (
      <div className="multiplePhotoStandard">
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <label>
            Border (mm):
            <input type="number" min="0" max="50" value={borderMm} onChange={(e) => setBorderMm(Number(e.target.value))} style={{ width: 80 }} />
          </label>
          ...
          <label>
            ...
            />
            {frameOverlay && (
              <button onClick={() => setFrameOverlay(null)} className="buttons-multiple">Clear Frame</button>
            )}
          </label>
          <label>
            Frame scope:
          ...
          <label>
            ...
            />
            {decoOverlay && (
              <button onClick={() => setDecoOverlay(null)} className="buttons-multiple">Clear Deco</button>
            )}
          </label>

          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <input placeholder="Template name" id="templateNameInput" style={{ width: 160 }} />
            <button
              ...
              onClick={() => {
                ...
                const tpl = { name, layout: layoutState, shotsCount, frameOverlay, decoOverlay, borderMm, showGuides, frameScope };
            ...
            <button
              className="buttons-multiple"
              onClick={() => {
                setTemplates((t) => t.filter((x) => x.name !== selectedTemplate));
          ...
          <button onClick={() => startStrip()} disabled={running} className="buttons-multiple">
            {running ? `Capturing... (${currentIndex}/${getLayoutSlots().slots})` : "Start Strip"}
          </button>
